...
namespace Repository.Implementation
{
    public class BaseRepo<T> : IBaseRepo<T> where T : class
    {
        ...
        public async virtual Task<IEnumerable<T>> GetAllAsync(BaseSearchEntity<T> searchEntity)
        {
            ...
            //if (!string.IsNullOrWhiteSpace(searchEntity.sortBy))
            //{
            //    var sortingExpression = GetSortingExpression(searchEntity.sortBy);
            //    if (sortingExpression != null)
            //    {
            //        query = string.Equals(searchEntity.sortOrder, "desc", StringComparison.OrdinalIgnoreCase) ? query.OrderByDescending(sortingExpression) : query.OrderBy(sortingExpression);
        ...
        public  async Task<IEnumerable<U>> GetOthers<U>() where U : class
        {
            DbSet<U> dbset = _dbContext.Set<U>();
            IEnumerable<U> query =await dbset.ToListAsync();
            return  query;
        }
    }
}
